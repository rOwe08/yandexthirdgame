...
namespace MvR
{
	...
	public class BuildController : MonoBehaviour
	{
        ...
        // A vertical and horizontal highlight object you can use to show where a building will be placed
        public Transform highlightVertical;
        public Transform highlightHorizontal;
		...
		public void Start()
		{
            ...
            // If there is a vertical highlight, hide it
            if (highlightVertical && highlightVertical.gameObject.activeSelf == true) highlightVertical.gameObject.SetActive(false);

            // If there is a horizontal highlight, hide it
            if (highlightHorizontal && highlightHorizontal.gameObject.activeSelf == true) highlightHorizontal.gameObject.SetActive(false);
			...
			if ( Physics.Raycast( pointer.position, -Vector3.up, out hit, 100) ) 
			{
				if (hit.collider.tag == "Money")
				{
					...
					if ( hit.collider.GetComponent<Money>().soundPickup )
						GetComponent<AudioSource>().PlayOneShot(hit.collider.GetComponent<Money>().soundPickup);
				...
				if (sell)
				{
					...
					if (building)
					{
						...
						{
							if (soundSell)
								GetComponent<AudioSource>().PlayOneShot(soundSell);
		...
		public void PlaceBuilding()
		{
            ...
            // If there is a vertical highlight, position it at the building
            if (highlightVertical)
            {
                // Activate the highlight object
                if (highlightVertical.gameObject.activeSelf == false) highlightVertical.gameObject.SetActive(true);

                // Position the highlight object at the position of the current building position
                highlightVertical.position = new Vector3(0, 0, currentBuild.position.z);
            }

            // If there is a horizontal highlight, position it at the building
            if (highlightHorizontal)
            {
                // Activate the highlight object
                if (highlightHorizontal.gameObject.activeSelf == false) highlightHorizontal.gameObject.SetActive(true);

                // Position the highlight object at the position of the current building position
                highlightHorizontal.position = new Vector3(currentBuild.position.x, 0, 0);
            }
            ...
            if (Input.GetButtonUp("Fire1"))
            {
				...
				if (currentBuild.GetComponent<Animation>())    currentBuild.GetComponent<Animation>().Play();
