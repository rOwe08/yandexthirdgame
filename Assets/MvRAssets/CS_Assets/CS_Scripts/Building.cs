...
namespace MvR
{
	...
	public class Building : MonoBehaviour
	{
		...
		internal MoneyMaker moneyMaker;
		...
		public void Start()
		{
			...
			moneyMaker = GetComponent<MoneyMaker>();
			...
			if ( isDead == false )
			{
				...
				if( health <= 0 )
				{
					...
					else if ( animator )    
					{
						...
						Invoke("RemoveDead", animator.GetCurrentAnimatorStateInfo(0).length);
					}
				}
				...
				if ( GetComponent<Animation>() )
				{
					if ( !GetComponent<Animation>().isPlaying )
					{
						if ( idleAnimation )
						{
							if ( GetComponent<Animation>()[idleAnimation.name] )
								GetComponent<Animation>().Play(idleAnimation.name);
						}
					}
				}
				else if ( animator && !animator.GetCurrentAnimatorStateInfo(0).IsName("idle")  )
				{
					if ( moneyMaker && moneyMaker.animator && !moneyMaker.animator.GetCurrentAnimatorStateInfo(0).IsName("money") )
					{
						animator.Play("idle", -1, 0f);  
					}
					else if ( weapon && weapon.animator && !weapon.animator.GetCurrentAnimatorStateInfo(0).IsName("attack") )
					{
						animator.Play("idle", -1, 0f);  
					}
				}
