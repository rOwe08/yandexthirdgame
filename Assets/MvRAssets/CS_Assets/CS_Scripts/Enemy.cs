...
namespace MvR
{
	...
	public class Enemy : MonoBehaviour
	{
				...
				if (attackDelay > 0 || kamikaze > 0)
				{
					if (isAttacking == false)
					{
						...
						else if (attackTarget) // If there is a valid target, attack it
						{
							...
							if ( animator )    animator.Play("attack", -1, 0f);
						}
					}

					if ( GetComponent<Animation>() && !GetComponent<Animation>().isPlaying )
					{
						attackTarget = null;
					}
				}
		...
		internal void Die()
		{
			...
			else if ( GetComponent<Animation>() && GetComponent<Animation>().GetClip("die"))
			{
