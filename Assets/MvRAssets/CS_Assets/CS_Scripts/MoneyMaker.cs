...
namespace MvR
{
	...
	public class MoneyMaker : MonoBehaviour
	{
		...
		internal bool isCreatingMoney = false;
		...
		public void Update()
		{
			...
			if ( delayCount >= delay && isCreatingMoney == false )
			{
				...
				if ( GetComponent<Animation>() )
				{
					...
					{
						if ( GetComponent<Animation>()["attack"] )
							GetComponent<Animation>().Play("attack");
					}
					else
					{
						if ( GetComponent<Animation>()[moneyAnimation] )
							GetComponent<Animation>().Play(moneyAnimation);
					}
				}
				else if ( animator )
				{
					animator.Play("money", -1, 0f);
				}
			}
		}
		...
		internal void CreateMoney( float delay )
		{
			...
			if ( moneyObject )
			{
				...
				Transform newMoney = Instantiate( moneyObject, transform.position + new Vector3(spawnArea.y, -0.5f, spawnArea.x), Quaternion.identity) as Transform;
